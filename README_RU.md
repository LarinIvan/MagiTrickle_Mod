...
*   **Rule Conflicts**: Автоматическое обнаружение конфликтов между правилами (пересекающиеся домены, IP-подсети, вложенные диапазоны)
    *   Возможность отключения или удаления конфликтующих правил в окне конфликтов, а также быстрый переход к правилу
...
| Импорт списков (Lists Import) | Конфликты правил (Conflicts) |
|:---:|:---:|
...
Рекомендуется как для **Entware**, так и для **OpenWRT**.

Скрипт автоматически определит вашу платформу (Entware или OpenWRT), добавит репозиторий и подскажет команды для установки.
...
**Entware:**
```bash
/opt/etc/init.d/S99magitrickle start    # Запуск сервиса
...
**OpenWRT:**
```bash
/etc/init.d/magitrickle start    # Запуск сервиса
