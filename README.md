<p align="center">
  <img src="https://github.com/LarinIvan/MagiTrickle_Mod/blob/develop/img/banner.png" alt="MagiTrickle logo"/>
</p>
...
> Данная версия развивается независимо от оригинального проекта MagiTrickle (не не совсем).
> Используйте на свой страх и риск.
> Оригинальный проект: [gitlab.com/magitrickle/magitrickle](https://gitlab.com/magitrickle/magitrickle)

## Назначение

**MagiTrickle Mod** — расширенная версия утилиты MagiTrickle для точечной маршрутизации трафика.

### Отличия от оригинала (Mod Features)

#### 🚀 Производительность и Оптимизация
*   **Trie (Prefix Tree) Lookup**: Оригинал перебирает массив правил линейно (O(n)). Мод использует префиксное дерево (Trie), что позволяет мгновенно находить нужный интерфейс даже среди тысяч доменов.
// TODO: выбор режима работы (trie/linear)
...
#### ⚙️ Функционал
*   **Speedtest**: Встроенная утилита для замера скорости через конкретный туннель/интерфейс.
...
> **Почему так?** GitHub использует защищенное HTTPS соединение. Стандартный `wget` в роутере его не поддерживает, поэтому мы ставим `wget-ssl`. Это также необходимо для дальнейшей работы пакетного менеджера с нашим репозиторием.

После выполнения скрипта установите мод командой:
```bash
opkg update
...
Для запуска, рестарта и остановки сервиса спользуйте команды:
